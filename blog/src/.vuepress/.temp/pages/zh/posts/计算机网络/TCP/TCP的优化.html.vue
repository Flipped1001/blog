<template><div><h2 id="如何优化tcp" tabindex="-1"><a class="header-anchor" href="#如何优化tcp"><span>如何优化TCP？</span></a></h2>
<h4 id="tcp三次握手的性能提升" tabindex="-1"><a class="header-anchor" href="#tcp三次握手的性能提升"><span>TCP三次握手的性能提升？</span></a></h4>
<p>客户端优化：</p>
...
需要双方同时打开才能有效</p>
<h4 id="tcp四次挥手性能的提升" tabindex="-1"><a class="header-anchor" href="#tcp四次挥手性能的提升"><span>TCP四次挥手性能的提升？</span></a></h4>
<p>主动方的优化：</p>
<blockquote>
...
<blockquote>
<p>如果双方同时关闭连接会出现什么？</p>
<p><strong>双方在等待 ACK 报文的过程中，都等来了 FIN 报文。这是一种新情况，所以连接会进入一种叫做 CLOSING 的新状态，它替代了 FIN_WAIT2 状态</strong></p>
</blockquote>
<h4 id="tcp传输数据的性能提升" tabindex="-1"><a class="header-anchor" href="#tcp传输数据的性能提升"><span>TCP传输数据的性能提升？</span></a></h4>
...
<p>滑动窗口是如何影像传输速度的？
TCP报文发出去之后不会从内存中立马删除，因为重传的时候还需要用到</p>
<p>这样看来，只要进程能及时地调用 read 函数读取数据，并且接收缓冲区配置得足够大，那么接收窗口就可以无限地放大，发送方也就无限地提升发送速度。</p>
<p>这是不能可能的，因为网络的传输能力是有限的，当发送方依据发送窗口，发送超过网络处理能力的报文时，路由器会直接丢弃这些报文。因此，缓冲区的内存并不是越大越好。</p>
<p>如何确定最大传输速度？</p>
<figure><img src="@source/zh/posts/计算机网络/TCP/2024-05-16-20-35-40.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
</div></template>

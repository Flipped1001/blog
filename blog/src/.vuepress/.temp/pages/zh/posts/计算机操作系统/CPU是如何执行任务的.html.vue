<template><div><!-- more -->
<h2 id="cpu如何执行任务的" tabindex="-1"><a class="header-anchor" href="#cpu如何执行任务的"><span>CPU如何执行任务的</span></a></h2>
<h4 id="cache-伪共享是什么" tabindex="-1"><a class="header-anchor" href="#cache-伪共享是什么"><span>Cache 伪共享是什么？</span></a></h4>
<p>现在假设有一个双核心的 CPU，这两个 CPU 核心并行运行着两个不同的线程，它们同时从内存中读取两个不同的数据，分别是类型为 <code v-pre>long</code> 的变量 A 和 B，这个两个数据的地址在物理内存上是<strong>连续</strong>的，如果 Cahce Line 的大小是 64 字节，并且变量 A 在 Cahce Line 的开头位置，那么这两个数据是位于<strong>同一个 Cache Line 中</strong>，又因为 CPU Cache Line 是 CPU 从内存读取数据到 Cache 的单位，所以这两个数据会被同时读入到了两个 CPU 核心中各自 Cache 中。</p>
...
<h4 id="避免伪共享的方法" tabindex="-1"><a class="header-anchor" href="#避免伪共享的方法"><span>避免伪共享的方法</span></a></h4>
<p>因此，对于多个线程共享的热点数据，即经常会修改的数据，应该避免这些数据刚好在同一个 Cache Line 中，否则就会出现为伪共享的问题。</p>
<p>接下来，看看在实际项目中是用什么方式来避免伪共享的问题的。</p>
<p>在 Linux 内核中存在 <code v-pre>__cacheline_aligned_in_smp</code> 宏定义，是用于解决伪共享的问题。</p>
<img src="https://telegraph-image-30o.pages.dev/file/8010b59a94cbe9422ed28.png" width="500" height="200">
<p>从上面的宏定义，我们可以看到：</p>
<ul>
<li>如果在多核（MP）系统里，该宏定义是 <code v-pre>__cacheline_aligned</code>，也就是 Cache Line 的大小；</li>
...
<p>我们都知道，CPU Cache 从内存读取数据的单位是 CPU Cache Line，一般 64 位 CPU 的 CPU Cache Line 的大小是 64 个字节，一个 long 类型的数据是 8 个字节，所以 CPU 一下会加载 8 个 long 类型的数据。</p>
...
<img src="https://telegraph-image-30o.pages.dev/file/eaca7b1056802520f2112.png" width="400" height="200">
<p>另外，RingBufferFelds 里面定义的这些变量都是 <code v-pre>final</code> 修饰的，意味着第一次加载之后不会再修改， 又<strong>由于「前后」各填充了 7 个不会被读写的 long 类型变量，所以无论怎么加载 Cache Line，这整个 Cache Line 里都没有会发生更新操作的数据，于是只要数据被频繁地读取访问，就自然没有数据被换出 Cache 的可能，也因此不会产生伪共享的问题</strong>。</p>
<h3 id="cpu-是根据什么来选择当前要执行的线程。" tabindex="-1"><a class="header-anchor" href="#cpu-是根据什么来选择当前要执行的线程。"><span>CPU 是根据什么来选择当前要执行的线程。</span></a></h3>
<p>在 Linux 内核中，进程和线程都是用 <code v-pre>task_struct</code> 结构体表示的，区别在于线程的 task_struct 结构体里部分资源是共享了进程已创建的资源，比如内存地址空间、代码段、文件描述符等，所以 Linux 中的线程也被称为轻量级进程，因为线程的 task_struct 相比进程的 task_struct 承载的 资源比较少，因此以「轻」得名。</p>
<p>一般来说，没有创建线程的进程，是只有单个执行流，它被称为是主线程。如果想让进程处理更多的事情，可以创建多个线程分别去处理，但不管怎么样，它们对应到内核里都是 <code v-pre>task_struct</code>。</p>
<img src="https://telegraph-image-30o.pages.dev/file/590b7a88e8dc8d73ffbdb.png" width="400" height="250">
<p>所以，Linux 内核里的调度器，调度的对象就是 <code v-pre>task_struct</code>，接下来我们就把这个数据结构统称为<strong>任务</strong>。</p>
<p>在 Linux 系统中，根据任务的优先级以及响应要求，主要分为两种，其中优先级的数值越小，优先级越高：</p>
<ul>
<li>实时任务，对系统的响应时间要求很高，也就是要尽可能快的执行实时任务，优先级在 <code v-pre>0~99</code> 范围内的就算实时任务；</li>
<li>普通任务，响应时间没有很高的要求，优先级在 <code v-pre>100~139</code> 范围内都是普通任务级别；</li>
</ul>
<h4 id="调度类" tabindex="-1"><a class="header-anchor" href="#调度类"><span>调度类</span></a></h4>
<p>由于任务有优先级之分，Linux 系统为了保障高优先级的任务能够尽可能早的被执行，于是分为了这几种调度类，如下图：</p>
...
<h4 id="完全公平调度" tabindex="-1"><a class="header-anchor" href="#完全公平调度"><span>完全公平调度</span></a></h4>
<p>我们平日里遇到的基本都是普通任务，对于普通任务来说，公平性最重要，在 Linux 里面，实现了一个基于 CFS 的调度算法，也就是<strong>完全公平调度（*Completely Fair Scheduling*）</strong>。</p>
...
<p>虽然是普通任务，但是普通任务之间还是有优先级区分的，所以在计算虚拟运行时间 vruntime 还要考虑普通任务的<strong>权重值</strong>，注意权重值并不是优先级的值，内核中会有一个 nice 级别与权重值的转换表，nice 级别越低的权重值就越大。 于是就有了以下这个公式：</p>
<img src="https://telegraph-image-30o.pages.dev/file/c4f8e77989bdd6c33e810.png" width="700" height="100">
<p>你可以不用管 NICE_0_LOAD 是什么，你就认为它是一个常量，那么在「同样的实际运行时间」里，高权重任务的 vruntime 比低权重任务的 vruntime <strong>少</strong>，于是高权重的获得的实际运行时间自然就多了。</p>
<h4 id="cpu-运行队列" tabindex="-1"><a class="header-anchor" href="#cpu-运行队列"><span>CPU 运行队列</span></a></h4>
<p>一个系统通常都会运行着很多任务，多任务的数量基本都是远超 CPU 核心数量，因此这时候就需要<strong>排队</strong>。</p>
<p>事实上，每个 CPU 都有自己的<strong>运行队列（*Run Queue, rq*）</strong>，用于描述在此 CPU 上所运行的所有进程，其队列包含三个运行队列，Deadline 运行队列 dl_rq、实时任务运行队列 rt_rq 和 CFS 运行队列 cfs_rq，其中 cfs_rq 是用红黑树来描述的，按 vruntime 大小来排序的，最左侧的叶子节点，就是下次会被调度的任务。</p>
<img src="https://telegraph-image-30o.pages.dev/file/cd00eda1dc993a1061818.png" width="400" height="300">
<p>这几种调度类是有优先级的，优先级如下：Deadline &gt; Realtime &gt; Fair，这意味着 Linux 选择下一个任务执行的时候，会按照此优先级顺序进行选择，也就是说先从 <code v-pre>dl_rq</code> 里选择任务，然后从 <code v-pre>rt_rq</code> 里选择任务，最后从 <code v-pre>cfs_rq</code> 里选择任务。因此，<strong>实时任务总是会比普通任务优先被执行</strong>。</p>
<h4 id="调整优先级" tabindex="-1"><a class="header-anchor" href="#调整优先级"><span>调整优先级</span></a></h4>
<p>如果我们启动任务的时候，没有特意去指定优先级的话，默认情况下都是普通任务，普通任务的调度类是 Fair，由 CFS 调度器来进行管理。CFS 调度器的目的是实现任务运行的公平性，也就是保障每个任务的运行的时间是差不多的。</p>
<p>如果你想让某个普通任务有更多的执行时间，可以调整任务的 <code v-pre>nice</code> 值，从而让优先级高一些的任务执行更多时间。nice 的值能设置的范围是 <code v-pre>-20～19</code>， 值越低，表明优先级越高，因此 -20 是最高优先级，19 则是最低优先级，默认优先级是 0。</p>
<p>是不是觉得 nice 值的范围很诡异？事实上，nice 值并不是表示优先级，而是表示优先级的修正数值，它与优先级（priority）的关系是这样的：priority(new) = priority(old) + nice。内核中，priority 的范围是 0~139，值越低，优先级越高，其中前面的 0~99 范围是提供给实时任务使用的，而 nice 值是映射到 100~139，这个范围是提供给普通任务用的，因此 nice 值调整的是普通任务的优先级。</p>
<img src="https://telegraph-image-30o.pages.dev/file/ca9b06907bc10f88f2a4d.png" width="400" height="150">
...
<p>如果想修改已经运行中的任务的优先级，则可以使用 <code v-pre>renice</code> 来调整 nice 值：</p>
...
<p><img src="https://telegraph-image-30o.pages.dev/file/01d414b6e78b646299f9a.png" width="400" height="100">!</p>
</div></template>
